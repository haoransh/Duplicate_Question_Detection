...
class QuoraProcessor(DataProcessor):
    ...
    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "qpairs_test.csv")), "test")
    ...
    def _create_examples(self, pd_data, set_type):
        ...
        for (i, line) in enumerate(pd_data.values):
            ...
            try:
                ...
                label = tokenization.convert_to_unicode(str(line[5]))
...
def get_dataset(processor,
    ...
    if mode == 'train':
        ...
        if not os.path.exists(eval_file):
            file_based_convert_examples_to_features(
                eval_examples, label_list, max_seq_length,
                tokenizer, eval_file)
        dataset = file_based_input_fn_builder(
        ...
        if not os.path.exists(test_file):
            file_based_convert_examples_to_features(
                test_examples, label_list, max_seq_length,
                tokenizer, test_file)
        dataset = file_based_input_fn_builder(

...
if __name__ == '__main__':
    ...
    parser.add_argument('--prediction-col', default='is_duplicate')
    arg = parser.parse_args()

    y_pred = pd.read_csv(
        arg.prediction_filepath,
        usecols=['id', arg.prediction_col]).set_index('id')
    y_pred_bin = (y_pred.values > 0.5).astype(np.int)
    y_true = pd.read_csv(
        arg.ground_truth_filepath,
        usecols=['id', 'is_duplicate']).set_index('id')
